...
!!! info
    The style guide is always a work in progress, and not all ConjugateGradient code
    follows the rules. When modifying ConjugateGradient, please fix the style violations
...
ConjugateGradient uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/singularitti/ConjugateGradient.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the ConjugateGradient directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to ConjugateGradient have
